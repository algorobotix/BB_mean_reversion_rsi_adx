"""
Оптимизатор стратегии: Backtrader + Optuna

Использование:
    python optimization/optimizer.py [--symbol BTCUSDT] [--trials 200] [--jobs 1]
                                     [--sampler tpe] [--objective composite]
                                     [--walk-forward] [--wf-folds 5] [--out-of-sample 0.20]

Примеры:
    python optimization/optimizer.py --trials 5 --objective composite --walk-forward --wf-folds 2
    python optimization/optimizer.py --symbol ETHUSDT --trials 300 --sampler tpe --objective sharpe
    python optimization/optimizer.py --symbol BTCUSDT --trials 500 --storage sqlite:///study.db
"""
...
def run_backtest_with_params(df: pd.DataFrame, params: dict, initial_cash: float = 1500.0) -> dict:
    from strategy.backtest import SimpleMeanReversionStrategy
    ...
    return compute_metrics(cerebro, strat, initial_cash, trades_df)
...
def detect_overfitting(train_m: dict, oos_m: dict, wf_df: pd.DataFrame) -> dict:
    ...
    if not wf_df.empty and 'return_pct' in wf_df.columns:
        ...
        if 'sharpe' in wf_df.columns:
            ...
            if wf_sh_std > 1.5:
                flags.append(f"Высокая вариативность Sharpe по фолдам (std={wf_sh_std:.2f})")
...
def save_results(study, best_params, train_m, oos_m, wf_df, overfit, symbol, args, out_dir: Path):
    out_dir.mkdir(parents=True, exist_ok=True)

    with open(out_dir / 'best_params.json', 'w', encoding='utf-8') as f:
        json.dump({'params': best_params, 'train_metrics': train_m,
    ...
    with open(out_dir / 'report.txt', 'w', encoding='utf-8') as f:
        ...
        lines += ['', f"OVERFITTING CHECK: {overfit['verdict']}"]
        if overfit['flags']:
...
def plot_results(study, wf_df: pd.DataFrame, out_dir: Path) -> None:
    try:
...
def build_sampler(name: str):
    seed = 42
    samplers = {
        'tpe':    TPESampler(n_startup_trials=20, multivariate=True, seed=seed),
        'cmaes':  CmaEsSampler(seed=seed),
        'nsga2':  NSGAIISampler(seed=seed),
        'random': RandomSampler(seed=seed),
    }
    return samplers.get(name, RandomSampler(seed=seed))


def print_comparison_table(default_params, best_params, default_m, best_train_m, oos_m):
    print('\n' + '=' * 75)
    print('СРАВНЕНИЕ: ДЕФОЛТНЫЕ vs ЛУЧШИЕ ПАРАМЕТРЫ')
    print('=' * 75)
    print(f"{'Параметр':<20} {'Дефолт':>12} {'Оптимум':>12}")
    print('-' * 45)
    for k in sorted(set(default_params) | set(best_params)):
    ...
    print('-' * 65)
    metrics_to_show = [
        ('return_pct', 'Return %'), ('sharpe', 'Sharpe'), ('sortino', 'Sortino'),
        ('calmar', 'Calmar'), ('sqn', 'SQN'), ('max_dd', 'Max DD'),
        ('profit_factor', 'Profit Factor'), ('win_rate', 'Win Rate'),
        ('total_trades', 'Trades'), ('expectancy', 'Expectancy'),
    ]
...
def parse_args():
    ...
    p.add_argument('--storage', default=None, help='sqlite:///study.db')
    return p.parse_args()
...
def main():
    ...
    from config.settings import interval, bb_lenth, bb_std, rsi_len, adx_len
    ...
    print("\nЗапуск бэктеста с дефолтными параметрами...")
    with suppress_output():
    ...
    if args.walk_forward:
        ...
        wf_df = walk_forward_validation(df_full, best_params,
